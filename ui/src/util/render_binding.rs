...
///
/// Starts a render binding program, which will update the specified layer any time the rendering binding changes
///
/// Returns the subprogram ID of the rendering program if it was successfully started
///
pub async fn run_render_binding(context: &SceneContext, render_layer: (NamespaceId, LayerId), rendering: impl Send + Into<BindRef<Vec<Draw>>>) -> Option<SubProgramId> {
    let Some(parent_program_id) = context.current_program_id() else { return None; };

    // Create a program ID for the render program
    let render_program_id   = SubProgramId::new();
    let rendering           = rendering.into();

    // Ask SceneControl to start the render program running
    let Ok(_) = context.send_message(SceneControl::start_child_program(SubProgramId::new(), parent_program_id, move |input, context| {
        render_binding_program(input, context, render_layer, Some(parent_program_id), rendering)
    }, 1)).await else { return None; };

    // Program was started, so return the program ID
    Some(render_program_id)
}
